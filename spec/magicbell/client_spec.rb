...
describe MagicBell::Client do
  ...
  describe '#create_notification' do
    ...
    context 'API response was not a 2xx response' do
      ...
      context('and there is a response body with an errors block') do
            ...
            [
              {
                ...
                'suggestion' => "Please provide the 'X-MAGICBELL-API-SECRET' header containing your MagicBell" \
                  " project's API secret. Alternatively, if you intend to use MagicBell's API in JavaScript in your" \
                  " web application's frontend, please provide the 'X-MAGICBELL-USER-EMAIL' header containing a" \
                  " user's email and the 'X-MAGICBELL-USER-HMAC' containing the user's HMAC.",
        ...
        it 'retries the default number of times on 5xx errors' do
          error = MagicBell::Client::HTTPError.new
          error.response_status = 500
          stub_request(:post, notifications_url).with(headers: headers, body: body).to_raise(error).then
                                                .to_raise(error).then
                                                .to_return(status: 200, body: {}.to_json)
          magicbell.create_notification(title: title)
        end

        it 'retries the default number of times on timeouts' do
          stub_request(:post, notifications_url).with(headers: headers, body: body).to_timeout.then
                                                .to_return(status: 408, body: response_body).then
                                                .to_return(status: 200, body: {}.to_json)
          magicbell.create_notification(title: title)
        end

        it 'retries the default number of times on timeouts and 5xx errors' do
          error = MagicBell::Client::HTTPError.new
          error.response_status = 500
          stub_request(:post, notifications_url).with(headers: headers, body: body).to_timeout.then
                                                .to_raise(error).then
      ...
      context('and there is a response body with with a non-array errors block') do
  ...
  describe 'Mark a user notification as read' do
    ...
    it 'can mark a user notification as read' do
      ...
      stub_request(:get, notification_url).with(headers: user_authentication_headers).and_return(status: 200,
                                                                                                 body: response_body)
  ...
  describe "Fetching a user's notifications" do
    ...
    context 'when the user is identified by external_id' do
      ...
      it "can fetch a user's notifications" do
        ...
        fetch_notifications_request = stub_request(:get, notifications_url).with(headers: user_authentication_headers)
                                                                           .and_return(status: 200, body: response_body)
  ...
  describe 'Mark a user notification as unread' do
    ...
    it 'can mark a user notification as unread' do
      ...
      fetch_notification_request = stub_request(:get, notification_url).with(headers: user_authentication_headers)
                                                                       .and_return(status: 200, body: response_body)
      user_notification = user.find_notification(notification_id)
      mark_notification_as_read_request = stub_request(:post, notification_read_url)
                                          .with(headers: user_authentication_headers)
  ...
  describe 'Mark all user notifications as read' do
    ...
    it "can mark all of a user's notifications as read" do
      magicbell = MagicBell::Client.new
      user = magicbell.user_with_email(user_email)
      mark_all_notifications_as_read_request = stub_request(:post, notifications_read_url)
                                               .with(headers: user_authentication_headers).and_return(status: 204)
  ...
  describe 'Mark all user notifications as seen' do
    ...
    it "can mark all of a user's notifications as seen" do
      magicbell = MagicBell::Client.new
      user = magicbell.user_with_email(user_email)
      mark_all_notification_as_seen_request = stub_request(:post,
                                                           notifications_seen_url).with(headers: user_authentication_headers).and_return(status: 204)
  ...
  describe 'Configuration' do
    it 'is also configurable in an initializer' do
      ...
      stub_request(:post, "#{api_host}/notifications").with(headers: headers, body: body).and_return(status: 201,
                                                                                                     body: '{}')
  ...
  describe 'API key and API secret configuration' do
    ...
    context 'No API key and API secret provided' do
      it 'keeps the global headers' do
        ...
        expect(magicbell.authentication_headers).to eq('X-MAGICBELL-API-KEY' => api_key,
                                                       'X-MAGICBELL-API-SECRET' => api_secret)
      end
    end
    ...
    context 'API key and API key provided' do
      it 'overrides the global headers with the project headers' do
        ...
        expect(magicbell.authentication_headers).to eq('X-MAGICBELL-API-KEY' => client_api_key,
                                                       'X-MAGICBELL-API-SECRET' => client_api_secret)
      end
    end
  end
